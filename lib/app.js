...
io.on('connection', socket => {
  ...
  socket.on('room:create', (data, err) => {
    if (!global.availableGames[data.game]) {
      err("don't hack")
      return
    }
    if (!data.creator || typeof data.creator !== 'string' || data.creator.length == 0) {
      err('Your name must not be empty!')
      return
    }
    if (!data.name || typeof data.name !== 'string' || data.name.length == 0) {
      err('Room name must not be empty!')
      return
    }
    if (!data.capacity || parseInt(data.capacity) < global.availableGames[data.game]. minCapacity || parseInt(data.capacity) > global.availableGames[data.game]. minCapacity) {
      err('Invalid room size!')
    }
      ...
      if(msg === true) {
        if (room.game.endTurn()) {
          ...
          setTimeout(() => {
            ...
            let before = room.game.scoreboard.length
            room.game.next()
            let after = room.game.scoreboard.length
            if (before != after) {  // round ended
              room.showScoreboard()
            }
            room.update()
          }, 5000)
